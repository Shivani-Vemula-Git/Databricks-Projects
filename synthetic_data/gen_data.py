...
def generate_data_for_sql_db():
    ...
    df_restaurants.to_csv(os.path.join(script_dir, "restaurants.csv"), index=False)
    df_menu_items.to_csv(os.path.join(script_dir, "menu_items.csv"), index=False)
    df_customers.to_csv(os.path.join(script_dir, "customers.csv"), index=False)
